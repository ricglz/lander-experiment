"""Module for creating and managing the surface"""
from random import random

import pygame

def random_ground(screen_height, screen_width, spacing, variation, landing_pad):
    """
    ...
    while i < number_of_points:
        ...
        if landing_pad[0][0] < last_x_point < landing_pad[1][0]:
            ans[-1] = ans[-2]
...
def build_landing_pad(width, height, screen_width, screen_height):
    # width in pixels
    buffer = screen_width * 0.05
    max_width, min_width = screen_width - buffer - width, buffer
    rand = random()
    starting_point = max_width - (rand * (max_width - min_width))
    return [
        (starting_point, screen_height - height),
        (starting_point + width, screen_height - height)
    ]

def get_landing_platform_rect(landing_pad_points, height):
    x, y = landing_pad_points[0][0], landing_pad_points[0][1] - height / 2
...
class Surface(pygame.sprite.Sprite):
    def __init__(self, screen_dimension):
        """
        @type screen_dimension: (int, int)
        """
        pygame.sprite.Sprite.__init__(self)
        # create the points for the polygon
        landing_pad_points = build_landing_pad(
            100, screen_dimension[1] * 0.1, screen_dimension[0], screen_dimension[1])
        self.polygon_points = random_ground(
                screen_dimension[1], screen_dimension[0], 20, 50, landing_pad_points)
        ...
        landing_pad_rect = get_landing_platform_rect(landing_pad_points, 20)
